...
\begin{enumerate}[leftmargin=*,nosep]
    ...
    \item \textbf{Hardware Feasibility Demonstration:} We developed a low-cost (\$50) single-channel EEG proof-of-concept prototype based on the ESP32 microcontroller and BioAmp EXG Pill. Feature importance analysis from research-grade data suggests that frontal channels carry sufficient discriminative information, providing biological rationale for this design; however, validation with hardware-acquired signals remains future work. % REVISION: Clarified hardware is proof-of-concept, not validated
\end{enumerate}

We emphasize that this work prioritizes \textit{methodological integrity} over state-of-the-art accuracy claims. The resulting 83.7\% subject-level accuracy, while modest compared to inflated literature benchmarks, represents an honest, reproducible baseline upon which future work can build. All classification results reported herein were obtained from research-grade EEG equipment; the hardware prototype serves as a proof-of-concept for future accessibility-focused validation studies. % REVISION: Added explicit statement about research-grade EEG
